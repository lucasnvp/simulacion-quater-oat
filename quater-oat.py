...
VALOR_PAQUETE_AVENA = 2800 * 25   # Entre 2800 y 3700 pesos x kg
...
def empaquetado(estados):
    ...
    kg_empaquetados = paquetes * 25
    
    if estados['stock_avena'] < kg_empaquetados:
        estados['stock_avena_empaquetada'] += math.ceil(estados['stock_avena']/25)
...
def ventas(estados):
    ...
    if estados['stock_avena_empaquetada'] > VD:
        ...
        estados['beneficio'] += VD * VALOR_PAQUETE_AVENA
        estados['stock_avena_empaquetada'] -= VD
    else:
        estados['ventas_totales'] += estados['stock_avena_empaquetada']
        estados['beneficio'] += estados['stock_avena_empaquetada'] * VALOR_PAQUETE_AVENA
...
def main(iterations: int):
    ...
    print(f"DIA_EMPAQUETADO_PERDIDO_X_FALTA_ST: {estados['dia_empaquetado_perdido_x_stock']}")
    print(f"DIA_EMPAQUETADO_PERDIDO_X_MAQUINA: {estados['dia_empaquetado_perdido_x_maquina']}")
    print(f"VENTA_PERDIDA_POR_FALTA_DE_PAQUETES: {estados['venta_perdida_por_falta_de_paquetes']}")
